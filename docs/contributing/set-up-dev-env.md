...
5. List the contents of the current directory (`/go/src/github.com/demonoid81/moby`).

   You should see the image's source from the  `/go/src/github.com/demonoid81/moby`
   directory.
...
9. Inside your container, check your Docker versions:
   ...
   * [docker/docker-ce](https://github.com/demonoid81/moby-ce) - The Docker CE
     edition project, which assembles engine, CLI and other components.
   
   The Moby project now defaults to a [fixed
   version](https://github.com/demonoid81/moby-ce/commits/v17.06.0-ce) of the
   ...
   ```
    ...
    project](https://github.com/demonoid81/moby-ce) and needs to be a Linux
    binary.
...
your Docker container.
   ...
   > **Note**: Inspecting the `Dockerfile` shows a `COPY . /go/src/github.com/demonoid81/moby` instruction, suggesting that dynamic code changes will _not_ be reflected in the container. However inspecting the `Makefile` shows that the current working directory _will_ be mounted via a `-v` volume mount.
